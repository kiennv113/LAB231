...
public class TakeQuizController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        int total = qdao.getNumQuestions();
        String mess = "";            
        if (numOfQuiz > total) {
            mess += "Please input number less than " + total;
            request.setAttribute("mess", mess);
            request.getRequestDispatcher("Home.jsp").forward(request, response);
        } else {
            QuizDAO qDAO = new QuizDAO();
            List<Quiz> test = qDAO.GetTest(numOfQuiz);
            int numberOfQuiz = test.size();
            request.setAttribute("test", test);
            request.setAttribute("size", numberOfQuiz);
                   HttpSession ses = request.getSession();
                   ses.setAttribute("numOfQuiz", numOfQuiz);
